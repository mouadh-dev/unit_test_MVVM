...
class ShoppingViewModelTest {
    ...
    @Test
    fun `insert wrong url picture, return error`(){
        viewModel.setCurImageUrl("http")
        val value = viewModel.insertShoppingItemStatus.getOrAwaitValueTest()
        assertThat(value.getContentIfNotHandled()?.status).isEqualTo(Status.ERROR)
    }

    @Test
    fun `insert right url picture, return success`(){
        viewModel.setCurImageUrl("http")
        val value = viewModel.insertShoppingItemStatus.getOrAwaitValueTest()
    ...
    fun `insert shopping item with empty field, returns error`() {
        viewModel.insertShoppingItem("mouadh","","3.0")
    ...
    fun `insert shopping item with too high amount, returns error`() {
        viewModel.insertShoppingItem("string","999999999999999999999999","3.0")
    ...
    @Test
    fun `insert shopping item with wrong url, returns error`() {
        viewModel.setCurImageUrl("ytytyt")
        val value = viewModel.insertShoppingItemStatus.getOrAwaitValueTest()
        assertThat(value.getContentIfNotHandled()?.status).isEqualTo(Status.ERROR)
    }
}